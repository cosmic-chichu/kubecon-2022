...
func BenchmarkPencil_Apply(b *testing.B) {
	img := setupPencil()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		pencil := effects.NewPencil(1)
		pencil.Apply(img, 2)
	}
}

func BenchmarkPencil_LoadApply(b *testing.B) {
	img, _ := effects.LoadImage("../test/turtle.jpg")
	pencil := effects.NewPencil(1)
	_, err := pencil.Apply(img, 2)
	require.Nil(b, err)
}

func setupPencil() *effects.Image {
	img, _ := effects.LoadImage("../test/turtle.jpg")
	return img
}
