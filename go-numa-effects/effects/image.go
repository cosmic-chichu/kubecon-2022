...
func createWrapperImage(img image.Image) (*Image, error) {
...
func LoadImage(path string) (*Image, error) {
	...
	return createWrapperImage(img)
}

// LoadImageBytes loads the bytes as an effects.Image. Supported file types are png and jpg
func LoadImageBytes(imgBytes []byte) (*Image, error) {
	img, _, err := image.Decode(bytes.NewBuffer(imgBytes))
	if err != nil {
		return nil, fmt.Errorf("failed to decode image on load: %s", err)
	}
